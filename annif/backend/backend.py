...
class AnnifBackend(metaclass=abc.ABCMeta):
    ...
    def _suggest_batch(self, documents, transform, params):
        ...
        for text in documents:
            text = transform.transform_text(text)
            hit_sets.append(self._suggest(text, params))
        return hit_sets
    ...
    def suggest_batch(self, documents, transform, params=None):
        """Suggest subjects for the input documents and return a list of subject sets
        represented as a list of SubjectSuggestion objects."""
        beparams = self._get_backend_params(params)
        self.initialize()
        return self._suggest_batch(documents, transform, params=beparams)
